...
void Twitterak::choice_login() //Showing diffrent oprtions after login
{
    ...
    while(flag == true)
    {
        ...
        else if(vec[0]=="follow")
        {
            ...
            if(users.count(strf) == 1)//checking this character exist
            { 
                users[temp]->add_following(strf) ;
                users[strf]->increase_follower() ;
                choice_login() ;
            }
            else
            {
 ...
 void Twitterak::show(string profile)
    {
       system("Color 0F");
        ...
        if(profile=="me" || profile=="profile")
        {
            ...
            cout << "$ follower : " ;
            cout << users[temp]->Get_followers() << endl ;
            cout << "$ following : " ;
            users[temp]->show_following() ;
              choice_login() ;
            }
        }
        ...
        else if(users.count(profile)==1)
        {
            ...
            cout << "$ follower : " ;
            cout << users[profile]->Get_followers() << endl ;
            cout << "$ following : " ;
            cout << users[profile]->Get_following() << endl ;
            choice_login() ;
        }
        else
        {
    ...
    void Twitterak:: edit_profile(string edit ,string changable)
    {
        ...
        if(users.count(temp)==1)
        {
            ...
            else if(edit == "password")//final change!!!!!!!!!
            {
                ...
                changable.erase(changable.size()-1, 1);
                hash<string>mystd;
                if(users[temp]->Get_Password()==mystd(changable))
                {
